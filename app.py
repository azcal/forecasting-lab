...
STATUS = {
    "WNBA": "live", "MLB F5": "live", "CS2": "live", "Soccer": "live (season opens mid-Aug)",
...
target whose recent performance drops below the base rate. Two candidate targets
failed validation and never shipped (NHL totals, an NCAAF lineup layer); one live
target (CS2 series length) is flagged and paused, by design. Early-season forecasts
on every board carry a calibration gate: predictions publish from day one, but the
board withholds actionability guidance until each league passes a games-played
threshold (e.g. NFL Week 5, NHL/NBA every team 9/8 GP).
""")
